...
autoBlock <- setRefClass(
    ...
    methods = list(
        ...
        runSpecListAndSaveBest = function(Rmodel, specList, name, auto=FALSE) {
            ...
            bestInd <- which(unlist(essPTminList) == max(unlist(essPTminList)))
            if(!is.null(names(specList))) name <- paste0(name, names(specList)[bestInd])

            it <<- it + 1
            naming[[it]] <<- name
            grouping[[it]] <<- determineGroupsFromSpec(specList[[bestInd]])
        ...
        determineCandidateGroupsFromCurrentSample = function() {
            ...
            candidateGroupsList <- lapply(cutreeList, function(ct) determineGroupsFromCutree(ct))
        ...
        determineNodeGroupSizesFromGroups = function(groups) {
            groupSizes <- numeric(0)
            for(gp in groups) for(node in gp) groupSizes[[node]] <- length(gp)
            return(groupSizes)
        },
